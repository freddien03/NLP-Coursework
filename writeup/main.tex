...
\textbf{Repository:} \url{https://github.com/freddienunn/NLP-Coursework}
...
Dev predictions (\texttt{dev.txt}) and test predictions (\texttt{test.txt}) were generated by the multi-task RoBERTa-base model described in Exercise 3, with a decision threshold of $\tau = 0.50$ selected by grid search on the dev set. The resulting dev F1 (positive class) is \textbf{0.6196}.
...
The best model improves on the baseline in 160 cases while regressing on only 41 (a 14.6$\times$ improvement-to-regression ratio), confirming that the overall F1 gain is genuine and broadly distributed rather than concentrated on a few examples.

\textbf{Only best model correct.}
The 160 cases where the best model succeeds but the baseline fails are dominated by examples annotated with \textit{Presupposition}, \textit{Compassion}, and \textit{Authority voice} categories — patterns the 1-epoch CE model misses entirely:
\begin{itemize}
    \item \textit{``Hollywood star Leo Di Caprio urges help for reuniting immigrant children with their families''} — gold=1, category: Unbalanced power relations. The construction ``[celebrity] urges help for [vulnerable group]'' positions the community as passive objects of advocacy, a structural pattern the multi-task encoder detects.
    \item \textit{``Rather sad. Good set of pictures that tells a tale of survival, subsistence living, and hopeless nature of life…''} — gold=1, categories: Presupposition, Metaphor, Compassion. The framing of poverty as immutable and ``hopeless'' is an implicit presupposition that longer focal-loss training brings into the model's representations.
\end{itemize}

\textbf{Only baseline correct / both wrong.}
The 41 regressions are mostly positive examples the best model under-predicts: e.g.\ \textit{``Our country is in need of serious change. We can continue to celebrate women's day, throw massive budgets at events…''} (category: Shallow solution), where sarcastic dismissal of superficial gestures is subtle. The 110 \textit{both wrong} cases are uniformly false negatives, with \textit{Presupposition} heavily represented — e.g.\ \textit{``Many refugees don't want to be resettled anywhere, let alone in the US''}, where PCL is embedded in a clause-level assumption rather than explicit condescending phrasing.

% -----------------------------------------------------------
\subsubsection*{Part B: Ablation Study}
% -----------------------------------------------------------

To isolate the contribution of the auxiliary category head, a single ablation was run: the same training setup (5~epochs, focal loss, full training set) but with the category head removed and only the binary PCL head trained. The optimal threshold for this ablation is $\tau=0.65$, compared to $\tau=0.50$ for the full model, reflecting a more conservative output distribution.
...
The ablation already achieves a substantial F1 of 0.6000, demonstrating that five epochs of focal-loss training alone accounts for most of the gain over the baseline (+0.0927 F1). The category head adds a further +0.0196 F1, but the nature of that improvement is more nuanced than the headline number suggests.

The key difference is in the precision/recall trade-off. The ablation is already very precise (P=0.6954) but under-predicts (R=0.5276): focal loss suppresses false positives effectively, but without the regularisation signal from the category head, the encoder is reluctant to commit to borderline PCL predictions. The category head, by forcing the encoder to represent the seven fine-grained PCL mechanisms simultaneously, pushes it to make more PCL predictions at a given confidence level. This closes the recall gap substantially (0.6181 vs.\ 0.5276) at a slight precision cost (0.6212 vs.\ 0.6954), and the net effect is a more balanced F1.

At the individual-example level the margin is narrow: the best model wins 26 cases the ablation misses, while losing 37 it gets right. The 26 best-model wins are mostly multi-category examples (e.g.\ Unbalanced power relations + Compassion, or Presupposition + Metaphor) where the encoder needs to simultaneously represent multiple PCL mechanisms. The 37 ablation wins are predominantly false positives that the full model avoids, consistent with its slightly lower precision. The 114 \textit{both wrong} cases largely overlap with the best-vs-baseline hard set, confirming that they are genuinely ambiguous examples that neither architectural choice resolves.
