r"""Ablation: single-task RoBERTa-base with focal loss (no category head).

Identical to model.py except the auxiliary category head and its BCE loss are
removed. Training uses only focal loss on the binary PCL label.
...
Compare dev F1 against model.py to quantify the contribution of the
category head as a multi-task regulariser.

Usage:
    python BestModel/ablation_no_category.py
"""
...
# ---------------------------------------------------------------------------
# Hyperparameters  (identical to model.py)
# ---------------------------------------------------------------------------
...
CHECKPOINT_DIR = REPO_ROOT / "checkpoints" / "roberta_no_category"
CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)


# ---------------------------------------------------------------------------
# Focal Loss  (identical to model.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Single-Task Model  (no category head)
# ---------------------------------------------------------------------------

class SingleTaskModel(nn.Module):
    """Transformer encoder with a single binary classification head."""

    def __init__(self, model_name: str = MODEL_NAME, prior_prob: float = 0.0948):
        super().__init__()
    ...
    def forward(
        ...
        return {"binary_logits": binary_logits}


# ---------------------------------------------------------------------------
# Dataset  (no category labels)
# ---------------------------------------------------------------------------

class PCLDataset(TorchDataset):
    """Dataset returning tokenized inputs and the binary PCL label only."""
...
# ---------------------------------------------------------------------------
# Custom Trainer with focal loss only  (no auxiliary BCE)
# ---------------------------------------------------------------------------

class SingleTaskTrainer(Trainer):
    """Trainer that computes focal loss on the binary head only."""
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        loss = self.focal_loss(outputs["binary_logits"], labels)
...
# ---------------------------------------------------------------------------
# Metrics & inference  (identical to model.py)
# ---------------------------------------------------------------------------

def compute_metrics(eval_pred):
    predictions, label_ids = eval_pred
    binary_logits = predictions[0] if isinstance(predictions, tuple) else predictions
...
def get_probs(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            outputs = model(**batch)
            probs = torch.softmax(outputs["binary_logits"], dim=-1)[:, 1].cpu().numpy()
...
def main():
    ...
    print(f"Class ratio: {n_neg} neg / {n_pos} pos = {n_neg/n_pos:.2f}:1")
    ...
    model = SingleTaskModel(MODEL_NAME, prior_prob=prior_prob)
    ...
    (CHECKPOINT_DIR / "train_info.json").write_text(
        json.dumps({
            "model": MODEL_NAME,
            "architecture": "single-task (binary only, no category head)",
    ...
    dev_out = predictions_dir / "ablation_no_category_dev.txt"
    ...
    test_out = predictions_dir / "ablation_no_category_test.txt"
